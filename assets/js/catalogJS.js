...
// clear-all
const choiceClear = document.querySelectorAll(".filter-choice__clear-all");
choiceClear.forEach((button) => {
  const id = button.id;
  button.addEventListener("mouseup", () => resetFilterChoice(id));
});

function resetFilterChoice(id) {
...
const rangeArray = [rangeFrom, rangeTo];

rangeSlider.noUiSlider.on("update", function (values, handle) {
  rangeArray[handle].value = Math.round(values[handle]);
});
...
rangeArray.forEach((el, index) => {
  el.addEventListener("change", (e) => {
    setRangeSlider(index, e.currentTarget.value);
  });
});

// LengthAllRangeSlider
const rangeSliderLengthAll = document.getElementById("range-length-all");

noUiSlider.create(rangeSliderLengthAll, {
  start: [90, 600],
  connect: true,
  step: 1,
  range: {
...
const rangeFromLengthAll = document.getElementById("range-from-length-all");
const rangeToLengthAll = document.getElementById("range-to-length-all");
const rangeArrayLengthAll = [rangeFromLengthAll, rangeToLengthAll];

rangeSliderLengthAll.noUiSlider.on("update", function (values, handle) {
  rangeArrayLengthAll[handle].value = Math.round(values[handle]);
});

const setRangeSliderLengthAll = (i, value) => {
  let arr = [null, null];
  arr[i] = value;
  rangeSliderLengthAll.noUiSlider.set(arr);
};

rangeArrayLengthAll.forEach((el, index) => {
  el.addEventListener("change", (e) => {
    setRangeSliderLengthAll(index, e.currentTarget.value);
  });
});

// LengthRangeSlider
const rangeSliderLength = document.getElementById("range-length");
...
const rangeFromLength = document.getElementById("range-from-length");
const rangeToLength = document.getElementById("range-to-length");
const rangeArrayLength = [rangeFromLength, rangeToLength];

rangeSliderLength.noUiSlider.on("update", function (values, handle) {
  rangeArrayLength[handle].value = Math.round(values[handle]);
});

const setRangeSliderLength = (i, value) => {
  let arr = [null, null];
  arr[i] = value;
  rangeSliderLength.noUiSlider.set(arr);
};

rangeArrayLength.forEach((el, index) => {
  el.addEventListener("change", (e) => {
    setRangeSliderLength(index, e.currentTarget.value);
  });
});

// WidthRangeSlider
const rangeSliderWidth = document.getElementById("range-width");
...
const rangeFromWidth = document.getElementById("range-from-width");
const rangeToWidth = document.getElementById("range-to-width");
const rangeArrayWidth = [rangeFromWidth, rangeToWidth];

rangeSliderWidth.noUiSlider.on("update", function (values, handle) {
  rangeArrayWidth[handle].value = Math.round(values[handle]);
});
...
rangeArrayWidth.forEach((el, index) => {
  el.addEventListener("change", (e) => {
    setRangeSliderWidth(index, e.currentTarget.value);
  });
});
const slidersTab = document.querySelectorAll('.noUi-handle');
slidersTab.forEach(function(sliderTab) {
  sliderTab.setAttribute('tabindex', '-1');
});


document.querySelector(".filter-catalog__title").addEventListener("click", function(e) {
  if (window.innerWidth < 701) {
    document.querySelector(".filter-catalog__wrapper").classList.toggle("filter-catalog__wrapper_active")
    document.querySelector(".filter-catalog__title_home").classList.toggle("filter-catalog__title_home_active")
    document.querySelectorAll(".filter-catalog__spoiler").forEach(item => {
        item.classList.toggle('filter-catalog__spoiler_active')
      });
  }
})
...
function spoiler(event) {
  if (
    !event.target.classList.contains("spoiler-open-btn_active") &&
    event.target.classList.contains("spoiler-open-btn")
  ) {
    spoilerOpenBtn.forEach((allSpoilerOpenBtn) => {
